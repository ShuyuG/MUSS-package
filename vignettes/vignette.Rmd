...
In high-dimensional sparse regression model, `MUSS` is developed for the cases when variables of the design matrix contain additive measurement errors. More specifically, we assume that the measurement errors are Normally distributed with mean 0, and errors for each variable have a specific variance. Inspired by SSLASSO[@doi:10.1080/01621459.2016.1260469] and EMVS[@doi:10.1080/01621459.2013.869223], here we also assign spike and slab priors to regression parameter $\boldsymbol{\beta}$. `MUSS` works under EM framework, where indicators of spike-slab priors and the unknown true design matrix are treated as latent variables. To avoid the effect of inappropriate choices of spike and slab scale parameters on variable selection, the final selected parameters are obtained following a decreasing sequence of spike parameters. The path of variable selection can be displayed by function from `MUSS` package.

**Notice**: Verified from simulation experiments, this method works when all the variables include additive errors and the variances are within a reasonable range. Satisfying selection results are not guaranteed when inappropriate initial values are chosen.

# Model

## Additive Measurement Error 
Suppose we have $n$ observations and $p$ potential predictors (possibly $p > n$, which is the high dimensional case), then the ordinary linear regression model is
$$
...
Assume each column of $X$ is hidden behind independent additive measurement errors. That is, what we actually observed is the matrix $Z$:
$$
Z = X + \Xi
$$
where the measurement errors are assumed to be independent Normal with mean $0$.
...
$$
Furthermore, it can be assumed that the prior of $\gamma_j$ is a $Bernoulli$ distribution depends on $\theta$:
$$
...
$$
and because of posterior $\pi(\boldsymbol{\beta},\theta,\sigma^2|\boldsymbol{y},Z, \Lambda)\propto f(\boldsymbol{y},\boldsymbol{\beta},\theta,\sigma^2|Z, \Lambda)$, now our goal is to find maximizers $\boldsymbol{\beta}$, $\theta$,$\sigma$ for
$$
...
First, we import the libraries to be used.
...
Then we use `MUSS` to select variables. `beta_prior_type` can be either `Laplacian` or `Gaussian`. `sigma_update = TRUE` by default. `return_g` means whether to return the maximized value of objective function $g$ at each iteration, and it is `FALSE` by default.  
...
The indices of nonzero coefficients are obtained. And the values of selected nonzero coefficients can be obtained by calling `$beta_values`.
