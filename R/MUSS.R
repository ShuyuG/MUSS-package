...
#'
#' @description
#' For high dimensional sparse regression with additive errors in potential variables,
#' Spike and Slab Variable Selector under Matrix Uncertainty(MUSS) selects variables under
#' EM framework by treating both unknown true variables and spike-slab indicators
#' as latent variables. Following a decreasing list of spike scale parameters, a path of
...
#'
#' @param beta_prior_type Character. Type of prior distribution of regression
#' parameter \eqn{\beta}. \code{beta_prior_type = "Laplacian"} or
#' \code{"Gaussian"}. Default is \code{"Laplacian"}.
#'
...
#' \code{theta ~ Beta(a,b)}. \code{a = 1} and \code{b = p} by default.
#'
...
#'
#' @export
MUSS = function(Z, y, tauList, beta_prior_type = "Laplacian",
                spike_params, slab_param, beta_init, sigma_update = TRUE,
                sigma_init, theta_init = 0.5, return_g = FALSE,
                a = 1, b, omega = 1, kappa = 1, tolerance = 0.01,
                max_iter = 500){
  ...
  if((beta_prior_type!="Laplacian") & (beta_prior_type!="Gaussian")) {
    stop("'beta_prior_type' should be either Laplacian or Gaussian!")
  }
  ...
  if(missing(spike_params)){
    ...
    if(beta_prior_type=="Laplacian"){
      spike_params = exp(seq(log(slab_param),by=-0.3,length.out=20))[-1]
    }else{
      spike_params = exp(seq(log(slab_param),by=-0.5,length.out=20))[-1]
    }
  }else{
  ...
  time_1 = Sys.time()
  if(beta_prior_type=="Laplacian"){
    MUSSfit = Laplacian_path(Z, y, tauList, spike_params, slab_param, beta_init,
                               sigma_update, sigma_init, theta_init, a, b,
                               omega, kappa, tolerance, max_iter, return_g)
  }else{
    MUSSfit = Gaussian_path(Z, y, tauList, spike_params, slab_param, beta_init,
                            sigma_update, sigma_init, theta_init, a, b,
                            omega, kappa, tolerance, max_iter, return_g)
  }
  MUSSfit$beta_indices = which(MUSSfit$beta_output!=0)
  MUSSfit$beta_values = MUSSfit$beta_output[MUSSfit$beta_indices]
  return(MUSSfit)
}
