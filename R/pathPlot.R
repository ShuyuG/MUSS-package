#' @title pathPlot
#' @description
#' Plot the path of \code{beta}, \code{theta} or \code{sigma}
...
pathPlot = function(fit_obj, path_of = "beta"){
  ...
  if (path_of == "beta"){
    ...
    if("beta_thresholds" %in% names(fit_obj)){
      Threshold = fit_obj$beta_thresholds
      Threshold_df = melt(data.frame(cbind(x_axis, Threshold)),id = "x_axis")
      Plot = Plot + geom_line(data = Threshold_df, aes(x = x_axis, y = Threshold,color = "Threshold",linetype = "Threshold"),size = 0.5)+
        geom_line(data = Threshold_df, aes(x = x_axis, y = -Threshold,color = "Threshold",linetype = "Threshold"),size = 0.5)+
        scale_color_manual("",values = c("Nonzero" = "brown","Zero" = "grey","Threshold" = "blue"))+ scale_linetype_manual("", values = c("Nonzero" = "solid","Zero" = "dashed","Threshold" = "dotted"))
    }else{
    ...
    print("path_of should be chosen from 'beta','theta' and 'sigma'.")
  }
}
